...
## Загрузка справочника операций из Excel

Справочник (`инит дата ... .xlsx`, лист «операции+сроки») загружается в таблицу
`static_initial_data_control_operations` скриптом `scripts/load_init_data.py`.
Лист «широкий» (девять колонок регионов), таблица «длинная» — скрипт разворачивает
регионы в отдельные строки и подставляет `t_material_id` / `region_id` из справочников БД.

```bash
py -m pip install -r scripts/requirements.txt

# проверить файл без подключения к БД
py scripts/test_load_init_data.py "инит дата 26 год (новый вариант).xlsx"

# проверить всё, включая справочники БД, но ничего не записывать
py scripts/load_init_data.py "инит дата 26 год (новый вариант).xlsx" --dry-run

# загрузить (TRUNCATE + вставка)
py scripts/load_init_data.py "инит дата 26 год (новый вариант).xlsx"
```

Зависимости операции задаются списками через запятую в `input_operation_order` /
`input_deadline` (например `90, 100` и `25, 30`); нулевой элемент — основная операция
на вход, последующие — альтернативные. Перед загрузкой применяется миграция из
`migrations/`.
